...
public class RoomScheduleDetailView: UIView, BaseView {
    ...
    private var hasFetchedRoomInfo = false
    ...
    private func refreshInfoRows() {
        ...
        if let password = roomPassword, !password.isEmpty {
            infoRowsStackView.addArrangedSubview(makePasswordRow())
        }
    ...
    // MARK: - Room info fetch

    private func fetchRoomInfo() {
        guard !hasFetchedRoomInfo else { return }
        hasFetchedRoomInfo = true
        roomStore.getRoomInfo(roomID: roomID) { [weak self] result in
            DispatchQueue.main.async {
                guard let self = self else { return }
                switch result {
                case .success(let info):
                    self.roomPassword = info.password
                case .failure:
    ...
    private func makePasswordRow() -> UIView {
        let row = UIView()
        row.snp.makeConstraints { $0.height.equalTo(Layout.rowHeight) }

        let labelLabel = makeRowLabel(.roomPassword)

        let valueLabel = UILabel()
        valueLabel.text = roomPassword
        valueLabel.font = RoomFonts.pingFangSCFont(size: 16, weight: .regular)
        valueLabel.textColor = RoomColors.valueText
        valueLabel.numberOfLines = 1
        valueLabel.textAlignment = .right
