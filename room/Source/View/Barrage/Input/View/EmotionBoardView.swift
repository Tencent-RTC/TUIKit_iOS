...
class EmotionBoardView: UIView {
    ...
    lazy var deleteButton: UIButton = {
        let button = UIButton()
        button.setImage(ResourceLoader.loadImage("room_barrage_delete")?.withTintColor(.white.withAlphaComponent(0.9)), for: .normal)
        button.setImage(ResourceLoader.loadImage("room_barrage_delete")?.withTintColor(.white.withAlphaComponent(0.14)), for: .disabled)
        button.addTarget(self, action: #selector(didSelectDeleteButton), for: .touchUpInside)
        button.layer.cornerRadius = 16.scale375Height()
        button.backgroundColor = RoomColors.copyButtonBackground
        return button
    }()
