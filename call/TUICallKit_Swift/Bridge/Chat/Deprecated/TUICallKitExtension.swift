...
class TUICallKitExtension: NSObject, TUIExtensionProtocol {
    ...
    func launchCall(type: CallMediaType, groupID: String, pushVC: UINavigationController, isClassic: Bool) {
        if !groupID.isEmpty {
    ...
    func getNavigationMoreItemExtensionForMinimalistChat(param: [AnyHashable : Any]?) -> [TUIExtensionInfo]? {
        ...
        let onClick: ([AnyHashable : Any]?, CallMediaType) -> Void = { [weak self] param, type in
            guard let self = self else { return }
