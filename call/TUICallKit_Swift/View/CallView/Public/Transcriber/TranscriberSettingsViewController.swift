...
final class TranscriberSettingsViewController: UIViewController {
    ...
    private lazy var pendingConfig: TranscriberConfig = TranscriberSettings.config
    ...
    private let transcribeStore: AITranscriberStore
    private var onDismiss: (() -> Void)?
    
    init(transcribeStore: AITranscriberStore, onDismiss: (() -> Void)? = nil) {
        self.transcribeStore = transcribeStore
    ...
    static func show(from viewController: UIViewController, transcribeStore: AITranscriberStore, onDismiss: (() -> Void)? = nil) {
        let settingsVC = TranscriberSettingsViewController(transcribeStore: transcribeStore, onDismiss:  onDismiss)
    ...
    @objc private func onBackButtonTapped() {
        let newConfig = pendingConfig
        let changed = hasConfigChanged()
        ...
        transcribeStore.updateRealtimeTranscriber(config: newConfig) { result in
            if case .success = result {
                DispatchQueue.main.async {
                    TranscriberSettings.config = newConfig
                }
            }
        }
    }
    
    private func hasConfigChanged() -> Bool {
        TranscriberSettings.config.sourceLanguage != pendingConfig.sourceLanguage
        || TranscriberSettings.config.translationLanguages != pendingConfig.translationLanguages
    }
    ...
    private func showLanguagePicker(for row: SettingRow) {
        ...
        switch row {
        case .sourceLanguage:
            pickerVC = LanguagePickerViewController.newInstanceForSource(selectedValue: pendingConfig.sourceLanguage.rawValue)
        case .translationLanguage:
            pickerVC = LanguagePickerViewController.newInstanceForTranslation(selectedValue:
                                                                                pendingConfig.translationLanguages.first?.rawValue ?? "")
        case .bilingual:
            return
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .sourceLanguage:
            return dequeueSettingCell(tableView, indexPath: indexPath,
                                      ...
                                      value: LanguageProvider.getSourceLanguageDisplayName(pendingConfig.sourceLanguage),
                                      showSeparator: !isLastRow)
        case .translationLanguage:
            return dequeueSettingCell(tableView, indexPath: indexPath,
                                      title: .translationLanguage,
                                      value: LanguageProvider.getTranslationLanguageDisplayName(pendingConfig.translationLanguages.first),
                                      showSeparator: !isLastRow)
        case .bilingual:
...
extension TranscriberSettingsViewController: LanguagePickerDelegate {
    func languagePickerDidSelect(value: String, isSourceLanguage: Bool) {
        ...
        if isSourceLanguage {
            ...
            pendingConfig = TranscriberConfig(
                sourceLanguage: sourceLanguage,
                translationLanguages: pendingConfig.translationLanguages
            )
            ...
            pendingConfig = TranscriberConfig(
                sourceLanguage: pendingConfig.sourceLanguage,
