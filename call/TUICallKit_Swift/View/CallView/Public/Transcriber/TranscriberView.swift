...
final class TranscriberView: UIView {
    ...
    private var tableViewTrailingConstraint: Constraint?
    ...
    func bindTranscriberStore(roomID: String) {
        ...
        guard currentRoomID != roomID else {
            updateMessages(AITranscriberStore.create(roomID: roomID).state.value.realtimeMessageList)
            return
        }
    ...
    private func refreshCallerLayout() {
        let isCaller = self.isCaller
        arrowImageView.isHidden = !isCaller
        tableViewTrailingConstraint?.update(offset: isCaller ? -28 : -12)
    }
    ...
    private func activateConstraints() {
        ...
        tableView.snp.makeConstraints { make in
            ...
            tableViewTrailingConstraint = make.trailing.equalToSuperview().offset(isCaller ? -28 : -12).constraint
        }
    ...
    @objc private func onContainerTapped() {
        guard isCaller, let viewController = findViewController(), let transcriberStore = transcriberStore else { return }
        TranscriberSettingsViewController.show(from: viewController, transcribeStore: transcriberStore) { [weak self] in
