...
/**
 * 2.12 流式 TTS 语音合成响应回调
 *
 * @param json_tts_response TTS 响应 JSON，字段定义见 @ref TTSResponse
 *        - kTIMTTSResponseMsgID：消息 ID
 *        - kTIMTTSResponseAudioDataList：本次下发的 Opus 音频分片列表，每个元素为 base64 编码的字符串
 *        - kTIMTTSResponseIsFinished：是否接收完毕
 * @param user_data ImSDK 负责透传的用户自定义数据
 */
typedef void (*TIMMsgTTSResponseCallback)(const char *json_tts_response, const void *user_data);
...
/**
 * 3.12 设置流式 TTS 语音合成响应回调
 *
 * @param cb 流式 TTS 响应回调，请参考 @ref TIMMsgTTSResponseCallback
 * @param user_data 用户自定义数据，ImSDK 只负责传回给回调函数 cb，不做任何处理
 */
TIM_API void TIMSetMsgTTSResponseCallback(TIMMsgTTSResponseCallback cb, const void* user_data);
 ...
 * 6.x 启动流式 TTS 语音合成
 *
 * @param msg_id            流式消息 ID
 * @param stream_msg_id     流式消息的 stream ID
 * @param ack_index         起始 ACK index（通常填 0；断点续拉时填已接收数）
 * @param json_voice_format 声音参数 JSON，字段定义见 @ref TTSVoiceFormat
 *        - kTIMTTSVoiceFormatVoiceID（可选）
 *        - kTIMTTSVoiceFormatSpeed（可选，合法范围 [0.5, 2.0]）
 * @param cb                启动 TTS 成功与否的回调
 * @param user_data         用户自定义数据
 */
TIM_API int TIMMsgStartTTS(const char* msg_id, const char* stream_msg_id, uint32_t ack_index,
                           const char* json_voice_format, TIMCommCallback cb, const void* user_data);
 ...
 * @param user_data      用户自定义数据
 */
TIM_API int TIMMsgStopTTS(const char* msg_id, const char* stream_msg_id, TIMCommCallback cb, const void* user_data);
...
//  - 客户端只能接收流式消息，不能发送
// string, 只读, 流式消息 ID
static const char* kTIMStreamElemStreamID = "stream_elem_stream_id";
...
//------------------------------------------------------------------------------
// 7.18 TTSVoiceFormat (流式 TTS 声音格式参数)
// string, 只写(选填), 音色 ID
static const char* kTIMTTSVoiceFormatVoiceID = "tts_voice_format_voice_id";
// float, 只写(选填), 播放速度，合法范围 [0.5, 2.0]，默认 1.0
static const char* kTIMTTSVoiceFormatSpeed = "tts_voice_format_speed";

//------------------------------------------------------------------------------
// 7.19 TTSResponse (流式 TTS 响应，由 @ref TIMMsgTTSResponseCallback 透出)
// string, 只读, 消息 ID（与 TIMMsgStartTTS 传入的 msg_id 对应）
static const char* kTIMTTSResponseMsgID = "tts_response_msg_id";
// array string, 只读, 本次下发的 Opus 音频分片列表，每个元素为 base64 编码的字符串
static const char* kTIMTTSResponseAudioDataList = "tts_response_audio_data_list";
// bool, 只读, 是否接收完毕（true 表示后续不会再有分片）
static const char* kTIMTTSResponseIsFinished = "tts_response_is_finished";
