 ...
 *
 *  @note 请注意如下特殊逻辑:
 * - 登录时票据过期：login 函数的 V2TIMFail 会返回 ERR_USER_SIG_EXPIRED（6206）或者 ERR_SVR_ACCOUNT_USERSIG_EXPIRED（70001） 错误码，此时请您生成新的 userSig 重新登录。
 ...
 * @note 该接口提供一些实验性功能
 */
- (NSObject* _Nullable)callExperimentalAPI:(NSString *)api
                                     param:(NSObject * _Nullable)param
                                      succ:(V2TIMCallExperimentalAPISucc _Nullable)succ
                                      fail:(V2TIMFail _Nullable)fail NS_SWIFT_NAME(callExperimentalAPI(api:param:succ:fail:));
@end
