...
/// Field description: the corner radius (in pixels) of the video image. After the upstream video stream is rendered according to `renderMode`, its final image is cropped with rounded corners.
/// Recommended value: default value: 0, which means no rounded corner processing.
///@note The value range is [0, min(width, height) / 2], i.e. no more than half of the length of the shorter side. This parameter takes effect only when `renderMode` is 0 (cropping mode).
...
/// `Description:` The corner radius (in pixels) of the video image. After the upstream video stream is rendered according to `fillMode`, its final image is cropped with rounded corners.
///`Value:` default value: 0, which means no rounded corner processing. Value range: [0, min(width, height) / 2], i.e. no more than half of the length of the shorter side.
@property(nonatomic, assign) int cornerRadius;
