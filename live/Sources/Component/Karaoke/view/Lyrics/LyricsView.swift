...
class LyricsView: UIView {
    ...
    private var lastDisplayedLine: LyricLine?
    ...
    func loadOnlineLyrics(lyricList: [TXLyricLine]) {
        lyricsData.removeAll()
        currentLineIndex = -1
        lyricsData = parseOnlineLyrics(lyricList: lyricList)
        updateLyrics(progress: 0)
    }

    private func parseLyricsFile(fileURL: URL) -> [LyricLine] {
        guard let lyricsInfo = LyricParser.parserLocalLyricFile(fileURL: fileURL) else {
            return []
        }

        var result: [LyricLine] = []

        for lineInfo in lyricsInfo.LyricLineInfos {
            let words = lineInfo.charStrArray.map { charInfo in
                LyricWord(text: charInfo.characterStr,
    ...
    private func parseOnlineLyrics(lyricList: [TXLyricLine]) -> [LyricLine] {
        var result: [LyricLine] = []

        for txLine in lyricList {
            let lineStartTime = Int(txLine.startTimeMs)
            let words = txLine.characterArray.map { txWord in
                LyricWord(
                    text: txWord.utf8Character,
                    startTime: Int(txWord.startTimeMs) - lineStartTime,
                    duration: Int(txWord.durationMs)
                )
            }
            ...
            if fullText.characterCount > 30 {
                ...
                let startTime = Int(txLine.startTimeMs)

                result.append(LyricLine(
                    startTime: startTime,
                    endTime: startTime + firstHalfDuration,
                    text: firstHalf.map { $0.text }.joined(),
                    words: firstHalf,
                    totalDuration: firstHalfDuration
                ))
                ...
                result.append(LyricLine(
                    startTime: startTime + firstHalfDuration,
                    endTime: startTime + firstHalfDuration + secondHalfDuration,
                    text: secondHalf.map { $0.text }.joined(),
                    words: secondHalf,
                    totalDuration: secondHalfDuration
                ))
                ...
                result.append(LyricLine(
                    startTime: Int(txLine.startTimeMs),
                    endTime: Int(txLine.startTimeMs) + totalDuration,
                    text: fullText,
                    words: words,
                    totalDuration: totalDuration
                ))
    ...
    private func findNextLine(after line: LyricLine) -> LyricLine? {
...
extension LyricsView: UITableViewDelegate, UITableViewDataSource {
