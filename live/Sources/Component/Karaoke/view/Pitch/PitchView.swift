...
class PitchView: UIView {
    ...
    private lazy var canvas: PitchCanvas = {
        let canvas = PitchCanvas()
    ...
    private var config: PitchViewConfig?
    private var cancellables = Set<AnyCancellable>()
    private var stdPitchModels: [PitchModel] = []
    private var hitPitchModels: [PitchModel] = []

    private var currentProgress: Int = 0
    private var currentPitch: Int = -1001
    ...
    private var currentScore: String = .score.liveLocalized
    ...
    func setConfig(config: PitchViewConfig) {
        self.config = config
        canvas.setConfig(config: config)
    }

    func setStandardPitchModels(standardPitchModels: [PitchModel]) {
    ...
    private func findPitchModel(pitchModels: [PitchModel], startTime: Int, endTime: Int) -> [PitchModel] {
    ...
    private func updateCurrentSingPitchAndHitPitchModelsIfNeededWithProgress(progress: Int,
                                                                             pitch: Int,
                                                                             stdPitchModelsOnView: [PitchModel]) {
    ...
    private func updateHitPitchModelsAtProgress(progress: Int, pitch: Int, matchedStdPitchModel: PitchModel) {
        guard let config = config else { return }
        ...
        if let prePitchModel = hitPitchModels.last {
            ...
            let model = PitchModel(startTime: startTime,
...
// MARK: - PitchCanvasDelegate
extension PitchView: PitchCanvasDelegate {
