...
class VRBattleInfoView: UIView {
    ...
    private lazy var singleBattleScoreView: VRSingleBattleScoreView = {
        let view = VRSingleBattleScoreView()
        view.isHidden = false
        return view
    }()
    ...
    private func updateBattleScore(battleScore: [String: UInt]) {
        ...
        let userList = coHostStore.state.value.connected.map(\.liveID)
        let firstUser = userList[0]
        let secondUser = userList[1]
        let leftSocre = battleScore[firstUser] ?? 0
        let rightSocre = battleScore[secondUser] ?? 0
        singleBattleScoreView.updateScores(leftScore: Int(leftSocre),rightScore: Int(rightSocre))
    }
}

extension VRBattleInfoView {
