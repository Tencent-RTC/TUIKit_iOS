...
struct LyricsInfo: Equatable {
    var LyricLineInfos: [LyricsLineInfo]
}

struct LyricsLineInfo: Equatable {
    var charStrArray: [LyricsCharacterInfo]
    var startTime: Double
    var endTime: Double
}

struct LyricsCharacterInfo: Equatable {
    let startTime: Int
    let duration: Int
    var endTime: Int {
...
public class LyricParser: NSObject {
    static var isVtt: Bool = false
    static var lyricsLineInfo: LyricsLineInfo = LyricsLineInfo(charStrArray: [],
                                                                     startTime: 0,
                                                                     endTime: 0)

    static func parserLocalLyricFile(fileURL: URL) -> LyricsInfo? {
        ...
        var lyricsInfo = LyricsInfo(LyricLineInfos: [])
        for str in lyricsArray {
                ...
                for lineStr in lyricsLineArray {
                    if lineStr.count > 0 {
                        ...
                        if isVtt {
                            ...
                            lyricsLineInfo.charStrArray.append(LyricsCharacterInfo(startTime: startTime,
                                                                                      duration: duration,
            ...
            if !isVtt && lyricsLineInfo.charStrArray.count > 0 {
                let lineLyricModelTemp = LyricsLineInfo(charStrArray: lyricsLineInfo.charStrArray,
