...
class AISubtitle: UIView {
    ...
    private func updateSubtitle() {
        DispatchQueue.main.async { [weak self] in
            ...
            self.hideTimer = Timer.scheduledTimer(withTimeInterval: self.showDuration, repeats: false) { [weak self] _ in
                guard let self = self else { return }
                self.isHidden = true
            }
        }
    ...
    private func getParticipant(participantId: String) -> CallParticipantInfo? {
        return CallStore.shared.state.value.allParticipants.first(where: { $0.id == participantId })
    }
    
    private func getDisplayName(participantId: String) -> String {
        if let participant = getParticipant(participantId: participantId) {
            if !participant.remark.isEmpty {
                return participant.remark
...
extension AISubtitle: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let displayName = getDisplayName(participantId: message.sender)
...
extension AISubtitle: TRTCCloudDelegate {
    func onRecvCustomCmdMsgUserId(_ userId: String, cmdID: Int, seq: UInt32, message: Data) {
        if let string = String(data: message, encoding: .utf8) {
            print("AISubtitle received: \(string)")
            if let data = string.data(using: .utf8),
               ...
               let type = json["type"] as? Int, type == aiMessageType {
                ...
                let translationLanguage = payload["translation_language"] as? String ?? ""
                
                if let index = translationInfo.firstIndex(where: { $0.roundId == roundId }) {
                    translationInfo[index].sender = sender.contains("TAI_Robot") ? translationInfo[index].sender : sender
                    translationInfo[index].text = text.isEmpty ? translationInfo[index].text : text
                    translationInfo[index].translation[translationLanguage] = translationText.isEmpty ? translationInfo[index].translation[translationLanguage] : translationText
                } else {
                    let info = TranslationInfo()
                    info.roundId = roundId
                    info.sender = sender.contains("TAI_Robot") ? "" : sender
                    info.text = text
                    info.translation[translationLanguage] = translationText
                    translationInfo.append(info)
                }
                updateSubtitle()
            }
        }
