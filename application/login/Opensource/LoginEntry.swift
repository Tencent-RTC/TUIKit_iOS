...
public final class LoginEntry: NSObject {
    ...
    #if LOGIN_FULL
    let debugAuthStore: any DebugAuthStoreProtocol = FullDebugAuthStore()
    #else
    let debugAuthStore: any DebugAuthStoreProtocol = OpensourceDebugAuthStore()
    #endif
        ...
        let launchAction: () -> Void = { [weak self] in
            ...
            if let loggedInMode = loggedInMode {
                ...
                case .phoneVerify, .emailVerify, .ioaAuth, .inviteCode:
                    ...
                    #else
                    LoginLogger.Login.warn("launchAction unsupported loggedInMode=\(loggedInMode) on OpenSource build")
                    self.clearLoggedIn()
                    #endif
                case .debugAuth:
    ...
    public func logoff(completion: @escaping (Result<Void, LoginError>) -> Void) {
        ...
        #else
        completion(.failure(.unknown(message: "Account delete is not available in the open-source build")))
        #endif
    }
    ...
    private func performLogout(completion: @escaping () -> Void) {
        ...
        #else
        OpensourceDebugAuthStore.clearCache()
        TUILogin.logout({ completion() }, fail: { _, _ in completion() })
        #endif
    }
