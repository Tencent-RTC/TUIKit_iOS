...
final class LoginNavigator: NSObject {
    ...
    #endif
    private var currentMode: LoginMode = .debugAuth
    ...
    func buildViewController(mode: LoginMode) -> UIViewController {
        ...
        #else
        case .phoneVerify, .emailVerify, .ioaAuth, .inviteCode, .menu:
        ...
        #if LOGIN_FULL && !RTCUBE_OVERSEAS && !RTCUBE_LAB
    ...
    private func pushDebugRegister(store: any DebugAuthStoreProtocol) {
        LoginLogger.Login.info("LoginNavigator.pushDebugRegister")
        let vc = UIViewController()
        vc.title = LoginLocalize("login_profile_title")
        let registerView = RegisterView()
        registerView.initialAvatar = store.state.avatarURL
    ...
    private func pushRegister(pendingResult: LoginResult) {
        ...
        if let userModel = LoginManager.shared.getCurrentUser() {
            registerView.initialAvatar = userModel.avatar
            if userModel.isMoa(), !userModel.name.isEmpty {
                registerView.initialNickName = userModel.name
            }
        }
        ...
        registerView.onRegisterButtonTapped = { [weak self] nickName, avatarURL in
            ...
            if let userModel = LoginManager.shared.getCurrentUser() {
                if !avatarURL.isEmpty {
                    ...
                    IMLogicRequest.synchronizUserInfo(currentUserModel: userModel, name: nickName, success: { _ in
