...
//  privacy
//
//  迁移自 v1 TRTCReportViewController，适配 v2 模块化架构
//  主要改动：
//    - TRTCReportLocalize → PrivacyLocalize
//    - TUICore Toast → 自定义 Toast（与 AntifraudAlertManager 一致）
//    - HttpLogicRequest.reportRoom → ReportNetworkService（模块内独立实现）
...
// MARK: - 对外调用入口（方案 A：保持 selector 方式兼容）
