...
//  AppAssembly
//
//  海外版（TencentRTC）Live 模块二级入口页 —
//  展示「视频直播」与「语聊房」两个 Cell，分别 push 到
//  `LiveListViewController` / `VoiceRoomViewController`。
//
//  从 iOS/BusinessService/AppScene/VideoLive/ui/LiveMainViewController 移植，
//  仅在 .overseas target 下作为 LiveModule 的目标页面。
//
...
private extension LiveEntranceViewController {
    ...
    @objc func helpClick() {
        // 海外版（TencentRTC）默认指向 trtc.io 文档；为保留与原 BusinessService 行为一致，
        // 仍按 isRTCube 兜底（防止 bundleId 与目标不匹配的边缘场景）。
