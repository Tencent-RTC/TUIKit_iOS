...
class RoomMessageViewModel: NSObject {
    ...
    private func enterRoom() {
        if !engineManager.store.isEnteredRoom {
            roomManager.enterRoom(roomId: message.roomId) { [weak self] _ in
                guard let self = self else { return }
                conferenceStore.dispatch(action: UserActions.updateSelfInfo(payload: UserInfo(userEntity: engineManager.store.currentUser)))
