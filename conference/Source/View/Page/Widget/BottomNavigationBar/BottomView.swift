...
#if RTCube_APPSTORE
        viewModel.getScreenSharedBannedStatus()
#endif
    }
    
    func getItemView(type: ButtonItemData.ButtonType) -> UIView? {
        let items = baseButtonMenuView.arrangedSubviews.compactMap { $0 as? BottomItemView }
        return items.first { itemView in
            itemView.itemData.buttonType == type
        }
    }
...
extension BottomView: BottomViewModelResponder {
    ...
    func showSeatTipsIfNeeded() {
        guard viewModel.isShownRaiseHandNotice else { return }
        guard let itemView = getItemView(type: .raiseHandItemType) else { return }
        layoutIfNeeded()
        DispatchQueue.main.async {
            BubbleTipsView.showBubble(.raiseHandNotice,
                                      baseView: itemView,
                                      containerView: self.superview,
                                      verticalSpacing: 10.0,
                                      config: BubbleTipsConfig.seatTipsConfig)
        }
    }
