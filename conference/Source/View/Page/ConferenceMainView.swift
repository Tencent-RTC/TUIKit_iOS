...
class ConferenceMainView: UIView {
    ...
    private var firstDelayDisappearanceTimer: Timer?
    private var delayDisappearanceTimer: Timer?
    ...
    private func bindInteraction() {
        firstDelayDisappearanceTimer?.invalidate()
        firstDelayDisappearanceTimer = Timer.scheduledTimer(withTimeInterval: firstDelayDisappearanceTime, repeats: false) { [weak self] _ in
    ...
    deinit {
        firstDelayDisappearanceTimer?.invalidate()
        delayDisappearanceTimer?.invalidate()
        unsubscribeEvent()
...
extension ConferenceMainView: ConferenceMainViewResponder {
    ...
    func changeToolBarHiddenState() {
        ...
        if topView.isHidden {
            showToolBar()
            startDelayHideToolBarTimer(delay: delayDisappearanceTime)
    ...
    private func startDelayHideToolBarTimer(delay: TimeInterval) {
        delayDisappearanceTimer?.invalidate()
        delayDisappearanceTimer = Timer.scheduledTimer(withTimeInterval: delay, repeats: false) { [weak self] _ in
            guard let self = self else { return }
            self.hideToolBar()
        }
    }
