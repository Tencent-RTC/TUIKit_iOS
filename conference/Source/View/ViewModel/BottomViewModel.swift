...
class BottomViewModel: NSObject {
    ...
    var isShownRaiseHandNotice: Bool {
        return engineManager.store.isShownRaiseHandNotice
    }
    var isCalledFromShareScreen = false
    private var isShareScreenBanned = false
    ...
    func shareScreenAction(sender: UIButton) {
        guard !isShareScreenBanned else {
            showScreenBannedAlert()
            return
        }
    ...
    func getScreenSharedBannedStatus() {
        TUICore.callService("TUICore_AppLoginService", method: "TUICore_AppLoginService_GetUserBannedFeatures", param: [:]) {
            [weak self] code, message, param in
            guard let self = self else { return }
            if let isShareScreenBanned = param["screen_share"] as? Bool {
                self.isShareScreenBanned = isShareScreenBanned
            }
        }
    }
...
extension BottomViewModel {
    ...
    private func showScreenBannedAlert() {
        viewResponder?.showAlert(title: .friendlyReminder, message: .screenSharedBannedMessage, sureTitle: .contactUsText, declineTitle: .cancelStageText, sureBlock:  {
            if let url = URL(string: "https://im.cloud.tencent.com/s/cWSPGIIM62CC/cFUPGIIM62CF") {
...
extension BottomViewModel: RoomEngineEventResponder {
    func onEngineEvent(name: EngineEventCenter.RoomEngineEvent, param: [String : Any]?) {
        ...
        case .onInitialSelfUserInfo:
            ...
            reorderTheMoreItem()
            viewResponder?.updateStackView(items: viewItems)
            viewResponder?.showSeatTipsIfNeeded()
...
private extension String {
    ...
    static let friendlyReminder = localized("Friendly Reminder")
    static let screenSharedBannedMessage = localized("If you would like to experience this feature, please contact us.")
    static let contactUsText = localized("Contact us")
}
